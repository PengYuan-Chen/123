    ...
    <div class="content">
      ...
      >
        ...
        <p @click="changeDefault(address, addressInfo)">
          ...
          <span class="s3" v-show="address.isDefault == 1">默認地址</span>
        </p>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      addressInfo: (s) => s.trade.address,
      orderInfo: (s) => s.trade.orderInfo,
    }),
    userDefaultAddress() {
      //find():查找數組中符合條件的結果返回
  ...
  methods: {
    ...
    submitOrder() {
      ...
      //其餘6個參數
      let data = {
        consignee:  this.userDefaultAddress.consignee,    //買家姓名
        consigneeTel: this.userDefaultAddress.phoneNum,    //電話
        deliveryAddress: this.userDefaultAddress.fullAddress,    //地址
        paymentWay: "ONLINE",    //付款方式統線上
        orderComment:this.msg,    //補充
        orderDetailList:this.orderInfo.detailArrayList    //購買商品清單
      };
